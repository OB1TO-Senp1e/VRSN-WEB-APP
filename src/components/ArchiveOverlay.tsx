    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="chapter-ink fixed inset-0 z-[96] overflow-y-auto"
          ...
          <div className="shell py-20 md:py-24">
            <div className="flex items-end justify-between gap-8 border-b border-line pb-8">
              <div>
                ...
                <h2 className="display text-[clamp(2.25rem,7vw,4.5rem)] uppercase">
                  Archive
                </h2>
              </div>
              <button
            ...
            <ul className="mt-2">
              {entries.map((e, i) => (
                <motion.li
                  ...
                  className="group grid grid-cols-[auto_1fr_auto] items-baseline gap-x-5 border-b border-line py-5 transition-colors duration-500 hover:border-line-strong md:grid-cols-[3rem_minmax(0,1fr)_minmax(0,14rem)_4rem] md:gap-x-8"
                >
                  <span className="t-index text-muted-2 transition-colors duration-500 group-hover:text-accent">
                    {String(i + 1).padStart(2, '0')}
                  </span>
                  <span className="display text-[1.375rem] transition-transform duration-700 [transition-timing-function:cubic-bezier(0.16,1,0.3,1)] group-hover:translate-x-1.5 md:text-[1.75rem]">
                    {e.title}
                  </span>
